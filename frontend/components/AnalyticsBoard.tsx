...
export default function AnalyticsBoard({ chunks, originalTextLength, stats, isLearningMode = false }: AnalyticsBoardProps) {
    ...
    return (
      <div className="glass-panel p-4 rounded-xl border border-border bg-card/60 flex items-center space-x-4">
        ...
        <div className="relative h-20 w-20 shrink-0">
          ...
          {/* Centered value percentage or text level */}
          <div className={`absolute inset-0 flex items-center justify-center font-mono font-bold ${
            isLearningMode ? "text-[10px]" : "text-xs"
          }`}>
            {isLearningMode ? ratingText : `${value}%`}
          </div>
        </div>
  ...
  const textCoherenceRating = coherenceScore >= 75 ? "High" : coherenceScore >= 50 ? "Medium" : "Low";
  const textOverlapRating = overlapEfficiency >= 75 ? "Optimal" : overlapEfficiency >= 50 ? "Low" : "Bloat";
  const textRetrievalRating = estimatedRetrievalQuality >= 75 ? "Good" : estimatedRetrievalQuality >= 50 ? "Mod" : "Weak";

  return (
    <div className="space-y-6">
      {/* Question Header */}
      <div className="pb-1.5 border-b border-border/50">
        <h4 className="text-xs font-bold text-foreground">Is this chunking strategy good?</h4>
        <p className="text-[10px] text-muted-foreground italic leading-normal">
          Evaluate split coherence, overlap density, and estimated RAG search profile below.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <CircularGauge
          ...
          ratingText={textCoherenceRating}
          subtitle={
            coherenceScore >= 80
        ...
        <CircularGauge
          ...
          ratingText={textRetrievalRating}
          subtitle={
      ...
      {/* Charts Section - progressive disclosure */}
      {!isLearningMode && (
